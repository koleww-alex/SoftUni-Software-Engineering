from project.computer_types.desktop_computer import DesktopComputer
from project.computer_types.laptop import Laptop


class ComputerStoreApp:

    def __init__(self):
        self.warehouse = []
        self.profits = 0

    def build_computer(self, type_computer: str, manufacturer: str, model: str, processor: str, ram: int):
        if type_computer == 'Desktop Computer':
            desktop_computer = DesktopComputer(manufacturer, model)
            self.warehouse.append(desktop_computer)
            return desktop_computer.configure_computer(processor, ram)

        elif type_computer == 'Laptop':
            laptop = Laptop(manufacturer, model)
            self.warehouse.append(laptop)
            return laptop.configure_computer(processor, ram)

        else:
            raise ValueError(f'{type_computer} is not a valid type computer!')

    def sell_computer(self, client_budget: int, wanted_processor: str, wanted_ram: int):
        for computer in self.warehouse:
            if all([computer.price <= client_budget,
                    computer.processor == wanted_processor,
                    computer.ram >= wanted_ram]):
                self.profits += client_budget - computer.price
                self.warehouse.remove(computer)
                return f'{str(computer)} sold for {client_budget}$.'

        raise Exception("Sorry, we don't have a computer for you.")


